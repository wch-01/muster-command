...
19. Apply the event's award method and repeat-winner behavior to every loot item. Participants adding or editing loot cannot override these organizer-controlled settings.
...
26. Extend Discord `/mc loot add` with a documented structured format supporting category, name, quality, quantity, and packaging. Retain the existing comma-separated format as shorthand that creates **Other** items with quantity one. The website remains the preferred interface for complex entries.
27. Store all event-level loot settings in templates: loot claim window, accepted resource forms, resource instructions, general loot instructions, award method, and repeat-winner behavior. Templates configure loot behavior but do not contain actual loot items.
...
- The event creator controls the award method and repeat-winner behavior for the entire pool; individual loot items cannot override them.
